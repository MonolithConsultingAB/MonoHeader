...
1. Extract `monoheader-1.9.5-source.zip`.
...
git init
git add .
git commit -m "Initial MonoHeader 1.9.5 source"
git branch -M main
...
The workflow in `.github/workflows/release-check.yml` runs the full release gate
for pushes to `main` and for pull requests. Generated directories such as
